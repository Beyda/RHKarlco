...
<?php
	...
	class usuario
	{
		...
		function __construct($empleado, $user, $pass, $t_usuario, $nomenclatura, $num_contador)
		{
	...
	class usuarioM
	{
		...
		function __construct($pass, $t_usuario, $id, $v)
		{
	        $this->pass = $pass;
	        $this->t_usuario =  $t_usuario;
	        $this->id = $id;
		...
		public function mod()
		{
			...
			$mysqli->set_charset("utf8");
				...
				// AGREGAR USUARIO 
				$obtUser = "UPDATE `usuarios` SET `pass`='$this->pass',`id_tipous`=$this->t_usuario, `jefe`= NULL WHERE `usuario` = '$this->id'";
				$resul_user = $mysqli->query($obtUser);


				// SELECT USUARIO
				$sel_user = "SELECT `id_usuario` FROM `usuarios` WHERE `usuario` = '$this->id' and `pass`='$this->pass' and `id_tipous`=$this->t_usuario";
				$res_user = $mysqli->query($sel_user);
				$numrows = $res_user->num_rows;
